...
export default class Model {
  ...
  static async extractValueFromStorage() {
    if (!Model.extractInProgress) {
      ...
      await Model.getVibrationModeActive();
  ...
  static async saveVibrationMode(vibrationActive) {
    Model.vibrationActive = vibrationActive;

    try {
      valueToSave = vibrationActive.toString();
      await AsyncStorage.setItem("vibrationActive", valueToSave);
    } catch (e) {}
  }

  static async getVibrationModeActive() {
    try {
      const valueSaved = await AsyncStorage.getItem("vibrationActive");
      if (valueSaved !== null) {
        Model.vibrationActive = valueSaved == "true";
      }
    } catch (e) {}
  }
}
